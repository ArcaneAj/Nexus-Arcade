...
export class PriceResultDialogComponent {
    ...
    configureGraph() {
        ...
        const dataWorldHq = this.data.result.history.entries.filter(e => e.hq && e.worldName === this.data.result.world).map(e => {
            return {
        ...
        const dataWorldNq = this.data.result.history.entries.filter(e => !e.hq && e.worldName === this.data.result.world).map(e => {
            return {
                x: HoursAgoFromTimestampSeconds(e.timestamp),
                y: e.pricePerUnit
            }
        ...
        const dataHq = this.data.result.history.entries.filter(e => e.hq && !(e.worldName === this.data.result.world)).map(e => {
            return {
                x: HoursAgoFromTimestampSeconds(e.timestamp),
                y: e.pricePerUnit
            }
        ...
        const dataNq = this.data.result.history.entries.filter(e => !e.hq && !(e.worldName === this.data.result.world)).map(e => {
            return {
                x: HoursAgoFromTimestampSeconds(e.timestamp),
                y: e.pricePerUnit
            }
